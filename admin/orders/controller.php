...
function doInsert(){
	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
				...
				$remarks ='Pesananmu Telah Dikonfirmasi';
				$delivered = Date('Y-m-d');

		}elseif ($_GET['actions']=='deliver') {
							# code...
				$status	= 'Pengiriman';	
				$remarks ='Pesananmu Telah Dikirimkan';
				$delivered = Date('Y-m-d');
				 
		}elseif ($_GET['actions']=='cancel'){
			// $order = New Order();
				$status	= 'Dibatalkan';
				$remarks ='Pesanan Anda telah dibatalkan karena kurangnya komunikasi dan informasi yang tidak lengkap.';
		}
   ...
   if(isset($_GET['id'])){
    ...
    for($i=0;$i<$max;$i++){
      ...
      if($qty>0 && $qty<=9999){
        ...
        $_SESSION['fixnmix_cart'][$i]['price']=$price;
			...
			message("Keranjang telah diperbarui.", "success");
			redirect("index.php?view=add");
	...
	function processorder(){
 		...
 		// 	unset($_SESSION['alltot']);
			message("Sukses Membuat Pesanan Baru! ", "success"); 		 
