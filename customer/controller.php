...
function doInsert(){
	...
	function doDelete(){
		...
		//	$_SESSION['ORDEREDNUM'] = $_POST['ORDEREDNUM'];
			...
			$summary->ORDEREDSTATS 	= 'Tertunda';
			$summary->CLAIMEDDATE		= $_POST['CLAIMEDDATE'];
			$summary->ORDEREDREMARKS 	= 'Pesananmu Dalam Proses';
			$summary->HVIEW 			= 0	;
			$summary->create();
		  }
		...
		message("Sukses Membuat Pesanan!", "success"); 		 
		redirect(web_root."index.php?q=profile");
	...
	function processwishlist(){
		...
		if(isset($_GET['wishid'])){
		 ...
		 if (isset($res)){
		 		message("Produk Telah Dihapus Dari Favorit", "success"); 		 
			redirect(web_root."index.php?q=profile");
		 }
	...
	function addwishlist(){
		...
		if($maxrow>0){
				message("Produk Telah Ditambahkan Ke Favorit", "error"); 		 
				redirect(web_root."index.php?q=profile"); 
		}else{
				$query ="INSERT INTO `tblwishlist` (`PROID`, `CUSID`, `WISHDATE`, `WISHSTATS`)  VALUES ('{$proid}','{$id}','".DATE('Y-m-d')."',0)";
				$mydb->setQuery($query);
		...
		function doChangePassword(){
			...
			message("Password telah diubah!", "success");
			redirect(web_root.'index.php?q=profile');
			}
		}
